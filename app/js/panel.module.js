  ...
  var loadPanel = function(){
    ...
    switch (true) {
      case panel.displayType === 'parcel':
        ...
        document.querySelector('.parcel-info.rental-info').innerHTML = panel.tempHTML[0];
        document.querySelector('.info-container > .rental').innerHTML = panel.tempHTML[1];
        document.querySelector('.info-container > .not-rental').innerHTML = panel.tempHTML[2];
        document.querySelector('.info-container > .street-name').innerHTML = localParcelData.propstreetcombined;
          ...
          $.getJSON("https://services2.arcgis.com/qvkbeam7Wirps6zC/ArcGIS/rest/services/Rental_Inspections/FeatureServer/0/query?where="+ encodeURI("ParcelNo='"+this.featureData.properties.parcelno+"'")+"&objectIds=&time=&geometry=&geometryType=esriGeometryEnvelope&inSR=&spatialRel=esriSpatialRelIntersects&resultType=none&distance=0.0&units=esriSRUnit_Meter&returnGeodetic=false&outFields=ACTION_DESCRIPTION%2C+ParcelNo%2C+CSM_RECD_DATE&returnGeometry=true&multipatchOption=xyFootprint&maxAllowableOffset=&geometryPrecision=&outSR=&returnIdsOnly=false&returnCountOnly=false&returnExtentOnly=false&returnDistinctValues=false&orderByFields=&groupByFieldsForStatistics=&outStatistics=&resultOffset=&resultRecordCount=&returnZ=false&returnM=false&quantizationParameters=&sqlFormat=none&f=json&token=", function( Rental_Inspections ) {
            ...
            if(Rental_Inspections.features.length){
              if(Rental_Inspections.features[0].properties){
                tempParcelDataHTML[0] = '<article class="info-items"><span>RENTAL STATUS</span> ';
                switch (Rental_Inspections.features[0].properties.ACTION_DESCRIPTION) {
                  case 'Issue Initial Registration':
                    tempParcelDataHTML[0] += '<initial><strong>Certificate of Registration Issued on</strong></initial><br>'+ moment(Rental_Inspections.features[0].properties.CSM_RECD_DATE).format('MMM Do,YYYY') +'</article>';
                    break;
                  case 'Issue Renewal Registration':
                    tempParcelDataHTML[0] += '<initial><strong>Certificate of Registration Issued on</strong></initial><br>'+ moment(Rental_Inspections.features[0].properties.CSM_RECD_DATE).format('MMM Do,YYYY') +'</article>';
                    break;
                  default:
                    tempParcelDataHTML[0] += '<cofc><strong>Certificate of Compliance Issued on</strong></cofc><br>'+ moment(Rental_Inspections.features[0].properties.CSM_RECD_DATE).format('MMM Do,YYYY') +'</article>';
                }
              }else{
                tempParcelDataHTML[0] = '<article class="info-items"><span>RENTAL STATUS</span> ';
                switch (Rental_Inspections.features[0].attributes.ACTION_DESCRIPTION) {
                  case 'Issue Initial Registration ':
                    tempParcelDataHTML[0] += '<initial><strong>Certificate of Registration Issued on</strong></initial><br>'+ moment(Rental_Inspections.features[0].attributes.CSM_RECD_DATE).format('MMM Do,YYYY') +'</article>';
                    break;
                  case 'Issue Renewal Registration':
                    tempParcelDataHTML[0] += '<initial><strong>Certificate of Registration Issued on</strong></initial><br>'+ moment(Rental_Inspections.features[0].attributes.CSM_RECD_DATE).format('MMM Do,YYYY') +'</article>';
                    break;
                  default:
                    tempParcelDataHTML[0] += '<cofc><strong>Certificate of Compliance Issued on</strong></cofc><br>'+ moment(Rental_Inspections.features[0].attributes.CSM_RECD_DATE).format('MMM Do,YYYY') +'</article>';
                }
              }
              tempParcelDataHTML[1] = '<a href="https://app.smartsheet.com/b/form?EQBCT=f3d4f41a75624b6fb497daa71ef79810" target="_blank"><article class="form-btn">SUBMIT RENTAL COMPLAINT</article></a>';
              tempParcelDataHTML[2] = '';

            }else{
              console.log('not a rental');
              console.log(getTempDisplayType());
              tempParcelDataHTML[0] = '<article class="info-items"><span>RENTAL STATUS</span> Not a Rental</article>';
              tempParcelDataHTML[1] = '<a href="https://app.smartsheet.com/b/form?EQBCT=91c0d55e47064373835ce198802764e2" target="_blank"><article class="form-btn">REPORT SUSPECTED RENTAL</article></a>';
              tempParcelDataHTML[2] = '<a href="https://app.smartsheet.com/b/form?EQBCT=7b3746bd20a048a5919ae07bd9ed89de" target="_blank"><article class="form-btn">REGISTER MY RENTAL</article></a>';
